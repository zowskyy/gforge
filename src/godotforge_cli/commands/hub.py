...
from godotforge_core.hub.run_record import compute_proof_hash, fold_run, read_events, verify_chain
...
@click.pass_context
def report(ctx: click.Context, run_id: str, format: str) -> None:
    """Generate a human-readable or JSON report for a completed run.

    Reads the run record via fold_run, verifies the proof_hash matches
    compute_proof_hash, and emits an envelope with the canonical run data.
    """
    root = _resolve_root(ctx)
    try:
        verify_chain(root)
    ...
    if record.state.value != "finalized":
        reraise(
            ValueError(f"run {run_id} is not finalized (state: {record.state.value})"),
            code=ForgeExitCode.CONFIGURATION_FAILURE,
        )
        raise  # unreachable
    ...
    if computed_proof != record.proof_hash:
        reraise(
            ValueError(
                f"proof hash mismatch: stored={record.proof_hash} "
                f"computed={computed_proof}"
            ),
            code=ForgeExitCode.PATCH_CONFLICT,
        )
        raise  # unreachable
    ...
    # Markdown format
    lines: list[str] = []
    lines.append(f"# Hub Run Report: `{record.run_id}`")
    lines.append("")
    lines.append(f"**State:** `{record.state.value}`  ")
    lines.append(f"**Outcome:** `{record.outcome}`  ")
    lines.append(f"**Proof Hash:** `{record.proof_hash}`  ")
    lines.append("")
    lines.append("## Canonical Hashes")
    lines.append("")
    lines.append(f"- **Goal Hash:** `{record.goal_hash}`")
    lines.append(f"- **Manifest Hash:** `{record.manifest_hash}`")
    lines.append(f"- **Plan ID:** `{record.plan_id}`")
    lines.append(f"- **Plan Hash:** `{record.plan_hash or 'noop'}`")
    lines.append("")
    if record.artifact_hash:
        lines.append("## Artifact Hashes")
        lines.append("")
        for path, digest in sorted(record.artifact_hash.items()):
    ...
    if record.engine:
        ...
        lines.append("")
        eng = record.engine
        lines.append(f"- **Version:** `{eng.get('version', 'unknown')}`")
        lines.append(f"- **Flavor:** `{eng.get('flavor', 'unknown')}`")
        lines.append(
            f"- **Executable SHA-256:** `{eng.get('executable_sha256', 'unknown')}`"
        )
        lines.append("")
    if record.validation:
        ...
        lines.append(f"- **Mode:** `{val.get('mode', 'unknown')}`")
        lines.append(f"- **Status:** `{val.get('status', 'unknown')}`")
        ...
        lines.append("")
    lines.append("---")
    lines.append("*Generated by `godotforge hub report`*")
